...
const Home: React.FC<HomeProps> = ({ setups, onSetupSelect, onSaveSetup, onShareRig }) => {
  ...
  const featured = setups[0] || setups[0];
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
            {(['trending', 'newest', 'top rated'] as SortOption[]).map((option) => (
              <button 
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap capitalize ${
                  activeSort === option 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {sortedSetups.map((setup) => (
            <div 
              ...
              className="group flex flex-col bg-surface-dark border border-border-dark rounded-2xl overflow-hidden hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/5 transition-all cursor-pointer animate-in fade-in slide-in-from-bottom-2 duration-500"
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3 flex flex-wrap gap-1">
                   ...
                   <span className="px-2 py-1 bg-black/40 backdrop-blur-md text-white/80 text-[10px] font-bold rounded uppercase tracking-tighter">
                     {setup.updatedAt}
                   </span>
                </div>
